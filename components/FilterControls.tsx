    ...
    <div className="flex flex-col h-full w-full md:w-80 bg-slate-900/90 backdrop-blur-xl border-r border-slate-700 shadow-2xl z-20">
      ...
      <div className="p-5 border-b border-slate-800 flex-shrink-0">
        <div className="flex items-center gap-3 mb-1">
          <div className="p-1.5 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-lg shadow-lg shadow-cyan-900/40">
      ...
      <div className="relative flex-1 min-h-0">
        ...
        >
          ...
          {FILTERS.map((filter) => (
            <label
              key={filter.id}
              className={`group flex items-start gap-3 p-2.5 rounded-lg transition-all cursor-pointer border ${
      ...
      {/* Footer Actions */}
      <div className="p-4 border-t border-slate-800 bg-slate-900/95 space-y-3 flex-shrink-0 relative z-30 shadow-[0_-5px_15px_rgba(0,0,0,0.3)]">
        <button
          onClick={onGenerate}
          disabled={isGenerating || selectedFilters.length === 0}
          className={`w-full py-3 px-4 rounded-xl font-bold text-slate-900 shadow-lg transition-all transform flex items-center justify-center gap-2 group relative overflow-hidden ${
        ...
        <button
          ...
          className="w-full py-2 px-4 rounded-lg font-medium text-slate-500 hover:text-white hover:underline decoration-slate-500 underline-offset-4 transition-all text-xs"
        >
          Reset All
        </button>
      </div>
