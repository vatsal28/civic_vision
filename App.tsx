...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // 1. Check for Credit Balance in LocalStorage
    const storedCredits = localStorage.getItem('civic_vision_credits');
    const hasSeenIntro = localStorage.getItem('civic_vision_intro');
    ...
    if (storedCredits) {
      ...
      setCredits(1);
      localStorage.setItem('civic_vision_credits', '1');
      localStorage.setItem('civic_vision_intro', 'true');
    }

    // 2. Check for cached API Key (BYOK persistence)
    const cachedKey = sessionStorage.getItem('civic_vision_key');
    if (cachedKey) {
  ...
  useEffect(() => {
    localStorage.setItem('civic_vision_credits', credits.toString());
  }, [credits]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const resultBase64 = await generateIdealImage(base64Data, activeFilters, keyToUse);
      
      // Deduct credit only on success if Guest
      if (authMode === AuthMode.GUEST) {
  ...
  if (!authMode) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-900 p-6 relative overflow-hidden font-sans">
        ...
        <div className="w-full max-w-lg bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-2xl p-8 text-center border border-slate-700 relative z-10 animate-fade-in">
          <div className="w-16 h-16 bg-cyan-500/10 text-cyan-400 rounded-full flex items-center justify-center mx-auto mb-6 border border-cyan-500/20 shadow-[0_0_15px_rgba(6,182,212,0.2)]">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z" />
            </svg>
          </div>
          <h1 className="text-2xl font-bold text-white mb-2">Welcome to CivicVision</h1>
          <p className="text-slate-400 mb-8 text-sm">
            Visualize improved urban spaces using AI. Choose how you want to proceed.
          </p>

          <div className="space-y-4">
            {/* OPTION 1: FREE TRIAL / CREDITS */}
            <div className="p-4 rounded-xl border border-slate-700 bg-slate-800/50 hover:bg-slate-700/50 transition-colors cursor-pointer group" onClick={() => handleSelectAuthMode(AuthMode.GUEST)}>
               <div className="flex justify-between items-center mb-2">
                 <h3 className="font-bold text-white group-hover:text-cyan-400 transition-colors">Start Free Trial</h3>
                 <span className="text-xs bg-cyan-500/20 text-cyan-400 px-2 py-1 rounded-full">Recommended</span>
               </div>
               <p className="text-left text-xs text-slate-400">Get 1 free generation. Top up with affordable packs (₹49 for 10) anytime.</p>
            </div>

            <div className="relative flex py-2 items-center">
                <div className="flex-grow border-t border-slate-700"></div>
                <span className="flex-shrink mx-4 text-slate-500 text-xs">OR USE YOUR OWN KEY</span>
                <div className="flex-grow border-t border-slate-700"></div>
            </div>
            ...
            {window.aistudio ? (
              <button
                ...
                className="w-full py-3 px-4 bg-slate-700 hover:bg-slate-600 text-white font-bold rounded-lg transition-colors border border-slate-600"
              >
                Connect via Google AI Studio
              </button>
            ) : (
              <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                 <form onSubmit={handleManualKeySubmit} className="space-y-3">
                   <input 
                     ...
                     className="w-full bg-slate-800 border border-slate-700 text-white text-sm rounded-lg focus:ring-cyan-500 focus:border-cyan-500 block p-2.5 placeholder-slate-500 outline-none transition-colors"
                     required
                   />
                   <button 
                     type="submit"
                     className="w-full py-2.5 px-4 bg-slate-700 hover:bg-slate-600 text-white font-medium rounded-lg transition-colors border border-slate-600"
                   >
                     Use My API Key
                   </button>
                 </form>
                 <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-xs text-cyan-400 hover:underline mt-2 inline-block">Get a free key</a>
              </div>
            )}
          </div>
        </div>
